    ...
    <div className="flex min-h-0 flex-1 flex-col p-4">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-xs text-white/45">当前分类共 {tasksKeys.length} 个任务</p>
        </div>
      </div>
      ...
      <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-1">
        {tasksKeys.length === 0 ? (
          ...
          tasksKeys.map((taskId) => {
            const status = statusMap[tasks[taskId].status];
            return (
              <div
                key={tasks[taskId].id}
              ...
              >
                <div className="mb-3 flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    <div className="mt-1 flex items-center gap-2 text-xs text-white/45">
                      <span>{tasks[taskId].folderName}</span>
                      <span>•</span>
                      <span>{tasks[taskId].size}</span>
                    </div>
                  </div>
                  <div
                ...
                <div className="mt-2 flex items-center justify-between text-xs text-white/45">
                  <span>进度</span>
                  <span>{tasks[taskId].progress}%</span>
                </div>
              </div>
            );
          })
